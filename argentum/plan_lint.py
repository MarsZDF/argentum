...
try:
    ...
    raise ImportError(
        "PlanLinter requires optional dependencies. "
        "Install with: pip install 'argentum-agent[lint]'. "
        f"Missing: {e}"
    )
...
class PlanLinter:
    ...
    def lint(
        ...
        if not isinstance(plan, dict) or "steps" not in plan:
            issues.append(
                Issue("E999", "error", "Invalid plan format: missing 'steps' array", "plan")
            )
    ...
    def _validate_parameters(
        self, step: Dict[str, Any], tool_spec: Dict[str, Any], step_id: str, auto_fix: bool
    ) -> List[Issue]:
    ...
    def _validate_dependencies(
        self, dependencies: Dict[str, List[str]], steps: List[Dict[str, Any]]
    ) -> List[Issue]:
    ...
    def _check_unused_outputs(
        self, steps: List[Dict[str, Any]], step_outputs: Dict[str, List[str]]
    ) -> List[Issue]:
