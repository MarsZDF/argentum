...
def _sanitize_dashboard_config(config: 'DashboardConfig') -> 'DashboardConfig':
    ...
    if hasattr(config, 'refresh_interval_minutes'):
        config.refresh_interval_minutes = min(max(1, config.refresh_interval_minutes), 1440)  # 1 min to 24 hours
...
class CostExporter:
    ...
    def export_csv(self, filepath: str, config: ExportConfig = None, 
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for event in cost_data:
                row = [
                    event.get("timestamp", "").strftime(config.date_format) if config.include_timestamps else "",
                    event.get("agent_id", ""),
                    event.get("operation", ""),
                ...
                if config.include_agent_breakdown:
                    row.extend([
                        event.get("model", ""),
                        event.get("efficiency_score", 0.0)
                    ])
    ...
    def export_excel(self, filepath: str, config: ExportConfig = None,
    ...
    def export_pdf_report(self, filepath: str, config: ExportConfig = None,
    ...
    def generate_dashboard_url(self, config: DashboardConfig = None) -> str:
        ...
        dashboard_data = {
            ...
            "expires_at": datetime.now() + timedelta(hours=config.expiry_hours) if config.expiry_hours else None
        }
        ...
        base_url = "https://dash.argentum.ai" if config.public_access else "https://internal-dash.argentum.ai"
    ...
    def export_json(self, filepath: str, date_range: Optional[tuple] = None,
    ...
    def _generate_summary_data(self, cost_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"Metric": "Average Cost per Token", "Value": f"${total_cost/max(total_tokens, 1):.6f}"},
            {"Metric": "Number of Operations", "Value": str(len(cost_data))},
            {"Metric": "Most Expensive Operation", "Value": f"${max((e.get('cost', 0) for e in cost_data), default=0):.4f}"}
        ]
