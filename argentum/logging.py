...
def setup_logging(
    ...
    if format_type == "minimal":
        ...
        formatter = ArgentumFormatter(
            include_module=include_module, include_timestamp=include_timestamp
        )
...
class PerformanceLogger:
    ...
    def end_timer(self, operation: str, extra_context: Optional[Dict[str, Any]] = None) -> float:
        ...
        self.logger.info(
            f"Operation '{operation}' completed in {elapsed:.3f}s", extra={"context": context}
        )
...
class DebugLogger:
    ...
    def log_plan_lint_results(
        self, plan_id: str, errors: int, warnings: int, execution_time: float
    ):
...
def configure_logging_for_development() -> None:
    ...
    setup_logging(
        level=logging.DEBUG, format_type="standard", include_timestamp=True, include_module=True
    )
