...
class TokenCounter:
    ...
    def count_messages(
        self, messages: List[Dict[str, str]], max_output_tokens: int = 1000
    ) -> TokenUsage:
        """
    ...
    def verify(
        self, estimated: TokenUsage, actual_input: int, actual_output: int
    ) -> Dict[str, Any]:
        """
        ...
        input_error_pct = (
            (input_diff / estimated.input_tokens * 100) if estimated.input_tokens > 0 else 0
        )
        output_error_pct = (
            (output_diff / estimated.output_tokens * 100) if estimated.output_tokens > 0 else 0
        )
...
def estimate_cost(tokens: int, tokenizer_type: TokenizerType, is_output: bool = False) -> float:
    """
