...
from argentum import ContextDecay, Handoff, HandoffProtocol, StateDiff
    ...
    def generate_state(self, size="small"):
        ...
        if size == "small":
            ...
            return {
                "memory": {
                    "facts": [
                        "fact_" + "".join(random.choices(string.ascii_lowercase, k=20))
                        for i in range(1000)
                    ],
                    "sources": [f"source_{i}.pdf" for i in range(500)],
                    "metadata": {
                        f"meta_key_{i}": f"meta_value_{i}_{''.join(random.choices(string.ascii_lowercase, k=50))}"
                        for i in range(200)
                    },
                    "confidence_scores": {f"topic_{i}": random.random() for i in range(300)},
                },
                "goals": [
                    f"goal_{i}_{''.join(random.choices(string.ascii_lowercase, k=30))}"
                    for i in range(100)
                ],
                "context": {
                    f"context_{i}": "".join(random.choices(string.ascii_lowercase, k=100))
                    for i in range(500)
                },
                "tools_used": {f"tool_{i}": random.randint(0, 100) for i in range(50)},
                "reasoning_history": [
                    {"step": i, "thought": "".join(random.choices(string.ascii_lowercase, k=200))}
                    for i in range(200)
                ],
            }
    ...
    def test_large_context_decay_simulation(self):
        ...
        for turn in range(1000):
            ...
            if turn % 2 == 0:  # Facts learned
                decay.add(
                    f"fact_{turn}", f"learned_fact_{turn}", importance=random.uniform(0.4, 0.9)
                )

            # Regular context items
            decay.add(
                f"context_{turn}", f"context_data_{turn}", importance=random.uniform(0.2, 0.8)
            )
...
class TestHandoffPerformance:
    ...
    def test_json_serialization_performance(self, benchmark):
        ...
        handoff = Handoff(
            ...
            context_summary="Complex handoff with lots of data: "
            + "".join(random.choices(string.ascii_lowercase, k=1000)),
            artifacts=[f"large_artifact_{i}.json" for i in range(100)],
            confidence=0.85,
            metadata={
                "large_data": {
                    f"key_{i}": "".join(random.choices(string.ascii_lowercase, k=100))
                    for i in range(100)
                },
                "metrics": {f"metric_{i}": random.random() for i in range(50)},
                "config": {
                    "setting_" + str(i): random.choice([True, False, "auto"]) for i in range(30)
                },
            },
        )
    ...
    def test_json_deserialization_performance(self, benchmark):
        ...
        handoff = Handoff(
            ...
            confidence=0.75,
            metadata={
                f"key_{i}": f"value_{i}_{''.join(random.choices(string.ascii_lowercase, k=50))}"
                for i in range(50)
            },
        )
...
class TestPlanLintPerformance:
    ...
    def generate_plan(self, steps_count=10):
        ...
        for i in range(steps_count):
            step = {
                "id": f"step_{i}",
                "tool": random.choice(
                    ["fetch_data", "clean_data", "analyze_trends", "create_visualization"]
                ),
                "parameters": {
    ...
    def test_complex_dependency_analysis_performance(self):
        ...
        tool_specs = {
            "analyze_trends": {"parameters": {"data": {"type": "string", "required": True}}}
        }
...
class TestMemoryUsage:
    ...
    def test_state_diff_memory_efficiency(self):
        ...
        large_state = {
            ...
            "nested_data": {
                "level1": {
                    "level2": {f"deep_key_{i}": f"deep_value_{i}" for i in range(500)}
                    for _ in range(10)
                }
            },
        }
        ...
        ), f"StateDiff using too much memory: {memory_increase_mb:.1f}MB"
